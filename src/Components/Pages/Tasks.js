...
class Tasks extends Component {
  ...
  async componentDidMount() {
    const list = await getList().catch((err) =>
  ...
  async onItemChange(id) {
    ...
    this.setState(({ list }) => {
      return {
        list: list.map((el) =>
          el.id === task.id ? { ...el, isCompleted: task.isCompleted } : el
        ),
      };
    });
  }
    ...
    return (
      <div className="main">
        ...
        <div className="button_with_input">
          <StatusButton
            list={list}
            handleChangeSatus={this.handleChangeSatus}
          />
          <div className="input">
            <input
              type="text"
        ...
        <div className="footer">
          <div className="itemsleft">
            {this.state.list.filter((el) => !el.isCompleted).length} items left
          </div>
          <Buttons
